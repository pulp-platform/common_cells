...
#
# Description: Two-Phase CDC FIFO Simulation Sweep
...
build_dir="${CDC_FIFO_2PHASE_BUILDDIR:-${repo_root}/build/vsim/cdc_fifo_2phase}"
...
call_vsim() {
  ...
  shift
  echo "== ${name} =="
  "${vsim_cmd[@]}" -c -quiet cc_cdc_fifo_2phase_tb "$@" -do 'run -all; quit -f' |
    tee "${log_file}"
